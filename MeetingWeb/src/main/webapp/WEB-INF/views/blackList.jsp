...
<script type="text/javascript">
	...
	var changememberlist=new Array();
	var changeblacklist=new Array();
	var resultmember=new Array();
	var resultblack=new Array();
	$(function() {
	...
	function goBlack(){
		var id=$("select.member option:selected").text();
		$("select.member option:selected").remove();
		if(id!=""){
			var html="<option>"+id+"</option>";
			$("select.black").append(html);			
		}
		
				
	}
	function goMember(){
		var id=$("select.black option:selected").text();
		$("select.black option:selected").remove();
		if(id!=""){
			var html="<option>"+id+"</option>";
	...
	function powerUpdate(){		
		if(confirm('수정하시겠습니까?')){
			$('select.member>option').each(function(){
				changememberlist.push($(this).text());
			})
			
			$('select.black>option').each(function(){
				changeblacklist.push($(this).text());
			})
			
			
			for(var i=0; i<changememberlist.length; i++)
			{			
				var exist=false;
				for(var j=0; j<memberlist.length; j++)
				{			
					if(changememberlist[i]==memberlist[j]){
						exists=true;
						i++;
						j=0;
					}				
				...
				if(!exist){
					resultmember.push(changememberlist[i]);
				}
			}
		 	
			for(var i=0; i<changeblacklist.length; i++)
			{			
				var exist=false;
				for(var j=0; j<blacklist.length; j++)
				{			
					if(changeblacklist[i]==blacklist[j]){
						exists=true;
						i++;
						j=0;
					}				
				...
				if(!exist){
					resultblack.push(changeblacklist[i]);
				}
			}	
	...
	function goMain(){
		if(confirm('취소하시고 메인페이지로 돌아가시겠습니까?')){
			location.href="main";
		}
	}
...
<body>
	...
	<section id="contents">
		...
		<select size="30" style="width:300px" class="member" id="member">
		<c:forEach var="list" items="${list }"> 
		
		<c:if test="${ list.power eq 'MEMBER'}">
		<script>		
		memberlist.push("${ list.id }");
		</script>
		...
		<td>
		<button type="button" onclick="goBlack()">>>></button><br>
		<button type="button" onclick="goMember()"><<<</button>
		</td>		
		<td>
		
		<select size="30" style="width:300px" class="black">
		<c:forEach var="list" items="${ list }"> 		
		<c:if test="${ list.power eq 'BLACK'}">
		<script>				
		blacklist.push("${ list.id }");
		</script>
		<option value="">${ list.id }</option>
		</c:if>
		</c:forEach>
		...
		<tr>
		<button type="button" onclick="powerUpdate()">수정</button>
