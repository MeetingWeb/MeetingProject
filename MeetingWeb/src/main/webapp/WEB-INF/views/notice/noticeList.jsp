			...
			<div id="noticetable">
				<table class="table">
					...
					<c:forEach var="list" items="${ map.list }">
						<tr>
							...
							<td style="text-align: left;">
								<a href="read?num=${ list.num }">${ list.title }</a>
							</td>
							<td>${ list.id }</td>
							<td>${ list.cre_date }</td>
							<td>${ list.hit }</td>
					...
					<tfoot style="vertical-align: inherit;">
						<tr style="border-bottom: none" class="buttontr">
							<td colspan="5">
								...
								<p class="buttons">
									...
									<a style="cursor: pointer;" onclick="allList()">
										<img src="/NowMeetingWeb/resources/images/list.png">
									</a>
									<sec:authorize access="hasAuthority('ADMIN')">
										<a style="cursor: pointer;" onclick="writeForm()">
											<img src="/NowMeetingWeb/resources/images/write.png">
										</a>
									</sec:authorize>
								</p>
							...
							<c:choose>
								<c:when test="${map.condition eq 'normal'}">
									<c:if test="${ map.page.currPage > 3 }">
										<li>
											<a href="getPage?page=${ map.page.startPage-1 }">&laquo;</a>
										</li>
									</c:if>
								</c:when>
								<c:when test="${map.condition eq 'search'}">
									<c:if test="${ map.page.currPage > 3 }">
										<li>
											<a href="searchPage?key=${ map.key }&page=${ map.page.startPage-1 }">&laquo;</a>
										</li>
									</c:if>
								</c:when>
							</c:choose>
							...
							<c:choose>
								<c:when test="${map.condition eq 'normal'}">
									<c:if test="${ map.page.endPage < map.page.totalPage }">
										<li>
											<a href="getPage?page=${ map.page.endPage+1 }">&raquo;</a>
										</li>
									</c:if>
								</c:when>
								<c:when test="${map.condition eq 'search'}">
									<c:if test="${ map.page.endPage < map.page.totalPage }">
										<li>
											<a href="searchPage?key=${ map.key }&page=${ map.page.endPage+1 }">&raquo;</a>
										</li>
									</c:if>
								</c:when>
							</c:choose>
